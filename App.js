...
export default class App extends React.Component {
  ...
  getIdeas() {
    ...
    let airtableUrl = "https://api.airtable.com/v0/appA80sV2BxKWW0ln/Bucket%20List";

    // Needed for Airtable authorization, replace with your own API key
    let requestOptions = {
      headers: new Headers({
        'Authorization': 'Bearer keyLIMJpywiyIAOfR'
      })
    };
  ...
  upvoteIdea(data, secId, rowId, rowMap) {
    ...
    let airtableUrl = "https://api.airtable.com/v0/appA80sV2BxKWW0lnBucket%20List" + data.id;

    // Needed for Airtable authorization
    let requestOptions = {
      method: 'PATCH',
      headers: new Headers({
        'Authorization': 'Bearer keyLIMJpywiyIAOfR', // replace with your own API key
  ...
  downvoteIdea(data, secId, rowId, rowMap) {
    ...
    // Airtable API endpoint
    let airtableUrl = "https://api.airtable.com/v0/appA80sV2BxKWW0ln/Bucket%20List" + data.id;

    // Needed for Airtable authorization
    let requestOptions = {
      method: 'PATCH',
      headers: new Headers({
        'Authorization': 'keyLIMJpywiyIAOfR', // replace with your own API key
        'Content-type': 'application/json'
      }),
      body: JSON.stringify({
        fields: {
